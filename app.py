...
# ===== PROCESS SPECIAL MENTIONS DATA (with file uploads) =====
        ...
        if moocs_entries:
            ...
            for item in moocs_entries:
                upload_path = None
                srno = str(item.get('srno', 1))
                file_key = f"moocs_{srno}_file"
                if file_key in request.files:
                    file = request.files[file_key]
                    if file and allowed_file(file.filename):
                        try:
                ...
                if not upload_path:
                    ...
                    print(f"No new file for MOOCS srno '{srno}', using previous upload: {upload_path}")
                cursor.execute("""
                    INSERT INTO moocs (form_id, name, month, duration, completion, srno, upload)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                """, (
                    form_id,
                    item.get('name', ''),
        ...
        if swayam_entries:
            ...
            for item in swayam_entries:
                ...
                if not upload_path:
                    ...
                    print(f"No new file for SWAYAM srno '{srno}', using previous upload: {upload_path}")
                cursor.execute("""
                    INSERT INTO swayam (form_id, name, month, duration, completion, srno, upload)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                """, (
                    form_id,
                    item.get('name', ''),
